...
export async function POST(request: NextRequest) {
  try {
    // Create Supabase client - we'll use the anon key but disable RLS
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
    const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;
    
    // Create a client with auth bypassed for server-side operations
      ...
      try {
        ...
        if (bookTitle) {
          bookId = bookMap.get(bookTitle.toLowerCase()) || 
                   bookMap.get(bookTitle.replace(/'/g, "'").toLowerCase()) ||
                   bookMap.get(bookTitle.replace(/'/g, "'").toLowerCase());
        }
        ...
        const { data: newContentId, error: contentError } = await supabase
          .rpc('import_study_packet', {
            p_name: title,
            p_content: content,
        ...
        if (contentError) {
          results.failed++;
          results.errors.push({ title, error: contentError });
          continue;
        }

        const newContent = { id: newContentId };
